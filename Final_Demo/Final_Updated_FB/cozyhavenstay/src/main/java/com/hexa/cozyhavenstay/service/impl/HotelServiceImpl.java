...
public class HotelServiceImpl implements HotelService {
     ...
     * This method now acts as the primary search endpoint for HotelDetailsDto.
     * It leverages the existing findAvailableHotelsByLocationAndDates from the repository.
     */
    @Override
    @Transactional(readOnly = true)
    public List<HotelDetailsDto> getAllHotels(String city, LocalDate checkInDate, LocalDate checkOutDate) {
        List<Hotel> hotels;

        // Log parameters for debugging
        System.out.println("HotelServiceImpl: getAllHotels (filtered) called with:");
        System.out.println("  City: " + city);
        System.out.println("  Check-in Date: " + checkInDate);
        System.out.println("  Check-out Date: " + checkOutDate);

        // Check if any search parameters are provided
        boolean hasSearchParams = (city != null && !city.trim().isEmpty()) ||
        ...
        if (hasSearchParams) {
            ...
            // Use the optimized repository method if search parameters are provided.
            // Pass a default numRooms if not provided by the current API call (assuming 1 is minimum for availability).
            int defaultNumRooms = 1;
            hotels = hotelRepository.findAvailableHotelsByLocationAndDates(city, checkInDate, checkOutDate, defaultNumRooms);
            System.out.println("HotelServiceImpl: Using findAvailableHotelsByLocationAndDates. Found " + hotels.size() + " hotels.");
        } else {
            // If no search parameters, return all active hotels (general browse)
            hotels = hotelRepository.findAll().stream()
                         .filter(Hotel::getIsActive) // Only return active hotels for public view
                         .collect(Collectors.toList());
            System.out.println("HotelServiceImpl: No search parameters, returning all active hotels. Found " + hotels.size() + " hotels.");
        }

        // Map the filtered Hotel entities to HotelDetailsDto
    ...
    public List<HotelSearchResponseDto> searchHotels(
        ...
        // This method remains separate, designed to return HotelSearchResponseDto with specific search details.
        // It's called by /api/hotels/search if you keep that endpoint active.
        ...
        return foundHotels.stream().map(hotel -> {
            ...
            dto.setImageUrl("https://example.com/images/default_hotel_image.jpg"); // Placeholder image URL
            ...
            if (hotel.getHotelAmenities() != null && !hotel.getHotelAmenities().isEmpty()) {
                ...
                dto.setAmenities(List.of()); // Return empty list if no amenities
            }
    ...
    private BigDecimal calculateTotalFare(
        ...
        if (maxPeopleWithoutExtraCharge <= 0) {
            maxPeopleWithoutExtraCharge = 2; // Default if not specified or invalid
        }

        if (totalRequestedPeople > maxPeopleWithoutExtraCharge) {
            int extraPeople = totalRequestedPeople - maxPeopleWithoutExtraCharge;
            int chargeableAdults = Math.min(requestedAdults, extraPeople); // Prioritize adults for charge
            int chargeableChildren = Math.max(0, extraPeople - chargeableAdults); // Remaining extra people are children
