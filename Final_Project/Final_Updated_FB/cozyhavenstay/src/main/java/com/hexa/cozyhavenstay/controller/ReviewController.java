...
public class ReviewController {
 ...
 // New RESTful endpoint: GET /api/hotels/{hotelId}/reviews
    @GetMapping("/hotels/{hotelId}/reviews")
    @PreAuthorize("isAuthenticated()") // Or remove this if you want public access
    public ResponseEntity<?> getHotelReviewsRestful(@PathVariable Integer hotelId) {
        try {
            List<ReviewResponseDto> reviews = reviewService.getReviewsByHotelIdForPublicView(hotelId);
            if (reviews.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
